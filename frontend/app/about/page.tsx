...
const fallbackWoredas = Array.from({ length: 19 }, (_, index) => `Woreda ${String(index + 1).padStart(2, "0")}`);
...
export default async function AboutPage() {
  ...
  const displayedSubcities = subcities.length > 0 ? subcities : fallbackSubCities.map((name, index) => ({ id: index + 1, name }));
  const displayedWoredas = woredas.length > 0 ? woredas : fallbackWoredas.map((name, index) => ({ id: index + 1, name }));
  const displayedServices = services.length > 0 ? services : fallbackServices.map((name, index) => ({ id: index + 1, name }));
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 px-4 py-16 md:px-8 lg:grid-cols-[1.1fr_0.9fr] lg:py-24">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-4">
              ...
              <Link href="/track-application" className="inline-flex items-center rounded-xl bg-white px-6 py-3 font-black text-[#062f6f] shadow-xl hover:bg-slate-100">
                Track Application
              </Link>
            </div>
          </div>
            ...
            <div className="rounded-[1.5rem] bg-white p-5 text-slate-900">
              <div className="grid grid-cols-2 gap-4">
                {stats.map((item) => {
                  ...
                  return (
                    <div key={item.label} className="rounded-2xl border bg-slate-50 p-5 shadow-sm">
                      <Icon className="mb-4 h-8 w-8 text-emerald-600" />
                      <div className="text-4xl font-black text-[#063d91]">{item.value}</div>
              ...
              <div className="mt-5 rounded-2xl bg-gradient-to-r from-[#063d91] to-emerald-600 p-5 text-white">
                ...
                <h2 className="mt-2 text-2xl font-black">Less waiting. More transparency. Better governance.</h2>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-7xl px-4 md:px-8">
          ...
          <div className="grid gap-6 lg:grid-cols-[0.8fr_1fr]">
            <div className="rounded-3xl bg-[#063d91] p-8 text-white shadow-xl">
              ...
              <h3 className="mt-6 text-3xl font-black">{cityName}</h3>
          ...
          <div className="mt-10 rounded-3xl border bg-slate-50 p-6 shadow-sm">
            ...
            <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
              {displayedWoredas.map((woreda) => (
                <div key={woreda.id} className="rounded-xl border bg-white px-4 py-3 text-sm font-bold text-slate-700 shadow-sm">
                  {woreda.name}
                </div>
              ))}
            </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 md:px-8">
        ...
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {displayedServices.map((service) => (
            <div key={service.id} className="rounded-2xl border bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
