...
export default function OfficerApplicationDetailPage() {
  ...
  const actionMutation = useOfficerApplicationAction(id);

  async function runAction(action: "approve" | "reject" | "return" | "complete") {
    await actionMutation.mutateAsync({ action, remark });
    setRemark("");
    toast.success(`Application ${action} action completed`);
  }
      ...
      <Card className="rounded-3xl">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button onClick={() => runAction("approve")} disabled={actionMutation.isPending}>
              Approve / Forward
            </Button>
            <Button variant="outline" onClick={() => runAction("return")} disabled={actionMutation.isPending}>
              Return
            </Button>
            <Button variant="outline" onClick={() => runAction("complete")} disabled={actionMutation.isPending}>
              Complete
            </Button>
            <Button variant="destructive" onClick={() => runAction("reject")} disabled={actionMutation.isPending}>
              Reject
            </Button>
            <Button asChild variant="outline">
