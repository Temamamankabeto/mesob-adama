...
import { useApplicationForm, useApplyService } from "@/hooks/application/use-application";
...
export default function ApplyPage() {
  ...
  const administrativeLevel = searchParams.get("administrative_level") || "";
  const cityId = Number(searchParams.get("city_id") || 0);
  const subcityId = searchParams.get("subcity_id") ? Number(searchParams.get("subcity_id")) : null;
  const woredaId = searchParams.get("woreda_id") ? Number(searchParams.get("woreda_id")) : null;
  ...
  useEffect(() => {
    ...
    if (role !== "customer") {
      toast.error("Only customers can apply for public services.");
      router.replace(`/login?redirect=${encodeURIComponent(currentPath)}`);
    }
  }, [router, currentPath]);

  const hasValidSelection =
    ["city", "subcity", "woreda"].includes(administrativeLevel) &&
    cityId > 0 &&
    (administrativeLevel !== "subcity" || Boolean(subcityId)) &&
    (administrativeLevel !== "woreda" || Boolean(subcityId && woredaId));
  ...
  const groupedSteps = useMemo(() => {
    ...
    return [{ title: "Application Information", sections: [], fields }];
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (!hasValidSelection) {
      toast.error("Please select service location first.");
      ...
      <div className="mx-auto max-w-2xl p-6">
        <Card className="rounded-3xl">
          <CardContent className="space-y-4 p-8 text-center">
            ...
            <h1 className="text-xl font-bold">Select Location First</h1>
            <p className="text-muted-foreground">
              You must choose City Level, Subcity Level, or Woreda Level before applying.
            </p>
            <Button onClick={() => router.push("/services")}>Choose Service Location</Button>
          </CardContent>
        </Card>
      </div>
    );
      ...
      <div className="mx-auto max-w-2xl p-6">
        <Card className="rounded-3xl">
          <CardContent className="p-8 text-center text-muted-foreground">
            No active form found for this service.
          </CardContent>
        </Card>
      </div>
    );
        ...
        <Card className="mb-4 rounded-3xl">
          <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4 text-sm">
            <div className="flex items-center gap-2 text-muted-foreground">
              ...
              Applying at <strong className="capitalize text-foreground">{administrativeLevel}</strong> level
            </div>
              ...
              <div className="rounded-2xl border bg-muted/20 p-6">
                ...
                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                  {(current?.fields || [])
                    ...
                    .sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0))
                    .map((field) => (
                      <div key={field.id} className={field.width === "full" || !field.width ? "md:col-span-2" : ""}>
