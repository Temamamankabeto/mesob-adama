...
export type SidebarChildItem = { label: string; href: string; permission?: string; scopes?: string[] };
export type SidebarItem = { label: string; href?: string; icon: LucideIcon; permission?: string; scopes?: string[]; children?: SidebarChildItem[] };
...
const officerApplicationMenu: SidebarItem = { label: "Officer Applications", icon: ClipboardCheck, children: [{ label: "Application Queue", href: "/dashboard/officer/applications", permission: "service_applications.review" }] };
const customerApplicationMenu: SidebarItem = { label: "My Applications", icon: FileText, children: [{ label: "Application List", href: "/dashboard/my-applications", permission: "applications.own" }, { label: "Track Application", href: "/dashboard/track-application", permission: "applications.track" }] };
const systemMenu: SidebarItem = { label: "System", icon: Settings, scopes: cityOnly, children: [{ label: "Audit Logs", href: "/dashboard/audit-logs", permission: "audit_logs.read", scopes: cityOnly }] };

const adminSections = (role: AppRoleKey): SidebarSection[] => [s("Main", [dashboardItem(role)]), s("Management", [userManagementMenu, locationManagementMenu, serviceManagementMenu, windowManagementMenu]), s("Applications", [formBuilderMenu, applicationManagementMenu]), s("System", [systemMenu])];
const managerSections = (role: AppRoleKey): SidebarSection[] => [s("Main", [dashboardItem(role)]), s("Applications", [applicationManagementMenu])];
const officerSections = (role: AppRoleKey): SidebarSection[] => [s("Main", [dashboardItem(role)]), s("Applications", [officerApplicationMenu])];
const customerSections = (role: AppRoleKey): SidebarSection[] => [s("Main", [dashboardItem(role)]), s("Applications", [customerApplicationMenu])];
...
export function getSidebarForRole(role?: string | null): RoleSidebar { return sidebarConfig[normalizeRole(role)]; }
...
function scopeAllowed(scopes?: string[]) { return !scopes?.length || scopes.includes(currentScopeKey()); }
function childAllowed(child: SidebarChildItem, permissions: string[]) { return (!child.permission || permissions.includes(child.permission)) && scopeAllowed(child.scopes); }
function itemAllowed(item: SidebarItem, permissions: string[]) { return (!item.permission || permissions.includes(item.permission)) && scopeAllowed(item.scopes); }
export function filterSidebarByPermissions(roleSidebar: RoleSidebar, permissions: string[] = []): SidebarSection[] {
  return roleSidebar.sections.map((section) => ({ ...section, items: section.items.map((item) => {
  ...
  }).filter(Boolean) as SidebarItem[] })).filter((section) => section.items.length > 0);
}
